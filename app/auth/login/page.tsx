    ...
    <div className="min-h-screen flex items-center justify-center bg-[image:var(--gradient-soft)] px-4 py-12">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">登录你的账户继续创作</p>
        </div>

      {error && (
        <div className="mb-4 p-3 bg-red-50/50 border border-red-200 text-red-600 rounded-lg text-sm">
          {error}
        </div>
      )}
      ...
      <div className="mt-6">
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white/0 backdrop-blur-sm text-muted-foreground">或使用以下方式登录</span>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button
            ...
            disabled={loading}
            className="w-full inline-flex justify-center py-2.5 px-4 border border-border rounded-lg shadow-sm bg-white/50 hover:bg-white/80 text-sm font-medium text-foreground transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div className="mt-6 text-center text-sm">
        ...
        <Link href="/auth/register" className="text-[hsl(var(--brand-coral))] hover:text-[hsl(var(--brand-pink))] font-medium transition-colors">
          立即注册
        </Link>
      </div>
      </motion.div>
